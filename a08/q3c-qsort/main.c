...
void quick_sort_range(struct sos *seq, int first, int last) {
  ...
  char *pivot = malloc(sizeof(char) * (strlen(sos_item_at(seq, first)) + 1));
  strcpy(pivot, sos_item_at(seq, first));      // first element is the pivot
  ...
  sos_swap(seq, first, pos);   // put pivot in correct place
  free(pivot);
