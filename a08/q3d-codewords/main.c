...
char *codestring(const struct sos *codebook, const char *str) {
  ...
  for (int i = 0; i < strlen(str); i++) {
    ...
    if (strcmp(sos_item_at(codebook, index), "") == 0) {
      continue;
    }
    
    char *code = malloc(sizeof(char) * (strlen(sos_item_at(codebook, index)) + 1));
    strcpy(code, sos_item_at(codebook, index));
    result = realloc(result, sizeof(char) * (strlen(result) + strlen(code) + 2));
    strcat(result, code);
    strcat(result, " ");
    free(code);
  }

  if (sos_item_at(codebook, 0)[0] != '\0') {
    result = realloc(result, sizeof(char) * (strlen(result) + strlen(sos_item_at(codebook, 0)) + 1));
    strcat(result, sos_item_at(codebook, 0));
  } else {
